...
| `GET /auth/google`, `GET /auth/facebook`                   | Démarrage de la connexion Google/Facebook (BACK-505) — appelant anonyme par nature (pas encore de compte/JWT). `@Public()`, `@Throttle` dédié (10/min)                                                                                                                                                                                                                                                                                                                                                                                           |
| `GET /auth/google/callback`, `GET /auth/facebook/callback` | Callback OAuth de connexion (BACK-505), DISTINCT de `/social-accounts/oauth/callback` ci-dessus (celui-ci authentifie un utilisateur anonyme, ne lie jamais un compte pro). `@Public()`, `@Throttle` dédié (30/min, même trafic que le callback Meta). Protégé par un `state` à usage unique (nonce anti-CSRF, sans payload puisqu'il n'y a pas de contexte entreprise à restituer). Ne renvoie jamais de token dans l'URL de redirection : uniquement un code d'échange à usage unique de 60 secondes, consommé par `POST /auth/oauth/exchange` |
| `POST /auth/oauth/exchange`                                | Échange le code d'échange à usage unique contre la vraie paire de tokens (BACK-505) — appelé par le frontend juste après la redirection, jamais directement par le navigateur. `@Public()` (l'appelant n'a pas encore de session), `@Throttle` dédié (10/min)                                                                                                                                                                                                                                                                                    |
| `POST /auth/forgot-password`, `POST /auth/reset-password`  | Réinitialisation de mot de passe (BACK-506), appelant par nature non authentifié. `@Public()`, `@Throttle` dédié (5/min, même limite que login/register — vecteur d'énumération/spam). `forgot-password` ne révèle jamais si l'email existe (toujours 201) ; le token de réinitialisation est un secret aléatoire à usage unique consommé atomiquement dans Redis (`GETDEL`), jamais un JWT                                                                                                                                                      |
